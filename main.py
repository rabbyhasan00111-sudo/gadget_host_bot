#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════════════╗
# ║   ⚡  GADGET  PREMIUM  HOST   v3.0   ·   main.py                    ║
...
async def cb_wallet(cq: CallbackQuery):
    ...
    used, mx = db.get_slot_counts(cq.from_user.id)
    await _edit(
        cq,
        f"🪙 <b>Coin Wallet</b>\n"
        f"{utils.divider()}\n"
        f"💰  Balance:      <code>{coins:,}</code>\n"
...
async def cb_coin_hist(cq: CallbackQuery):
    ...
    lines = [f"📜 <b>Coin History (last {len(txs)})</b>\n"]
    for tx in txs:
        ...
        lines.append(
            ...
            f"<i>{tx['reason'][:30]}</i>  "
            f"<code>{str(tx['created_at'])[:16]}</code>"
        )
    b = InlineKeyboardBuilder()
...
async def cb_daily(cq: CallbackQuery):
    ...
    if not ok:
        next_claim = "tomorrow at midnight"
        await _edit(
            cq,
            f"🎁 <b>Daily Reward</b>\n\n"
            f"⏳ Already claimed today!\n\n"
            f"🔥 Streak:  <code>{streak} days</code>\n"
            f"🪙 Balance: <code>{bal:,}</code>\n\n"
            f"<i>Come back {next_claim}!</i>",
            kb.kb_home(),
        )
        return

    streak_bar = utils.pbar(streak % 7, 7)
    await _edit(
        cq,
...
async def cb_help(cq: CallbackQuery):
    ...
    await _edit(
        cq,
        f"❓ <b>Help  ·  {config.BOT_NAME} v{config.BOT_VERSION}</b>\n"
        f"{utils.divider()}\n\n"
        "<b>Commands:</b>\n"
        "/start — Dashboard\n"
        ...
        "<b>Earn Coins:</b>\n"
        f"🎁 Daily +{config.DAILY_BASE_COINS} base, streak bonuses\n"
        f"🔗 Referral +{config.REFERRAL_COINS}/friend\n"
...
async def on_startup() -> None:
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="server",      description="🖥️ Server stats (Admin)"),
        BotCommand(command="exec",        description="💻 Shell command (Owner)"),
        BotCommand(command="maintenance", description="🔧 Maintenance (Admin)"),
        BotCommand(command="broadcast",   description="📢 Broadcast (Admin)"),
        BotCommand(command="user",        description="🕵️ User profile (Admin)"),
        BotCommand(command="finduser",    description="🔍 Search user (Admin)"),
        BotCommand(command="setplan",     description="📋 Set plan (Admin)"),
        BotCommand(command="setslots",    description="🔲 Set slots (Admin)"),
        BotCommand(command="addcoins",    description="🪙 Add coins (Admin)"),
        BotCommand(command="about",       description="ℹ️ Bot info"),
    ])
    ...
    with suppress(Exception):
        await bot.send_message(
            config.OWNER_ID,
            f"⚡ <b>{config.BOT_NAME} v{config.BOT_VERSION} is ONLINE!</b>\n\n"
